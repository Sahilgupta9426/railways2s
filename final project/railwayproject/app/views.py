...
from railwayproject.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRET_KEY 
import razorpay
...
def status(request):
    ...
    if status1==True:
        ...
        savecustomer=Booking(name=customer['name'],age=customer['age'],gender=customer['gender'],number=customer['number'],email=customer['email'],seat_no=train_seat1,amount=customer['amount']/100,journey_date=customer['date'],s_id=train_no,p_status=True,user=usr,order_id= response['razorpay_order_id'])

        savecustomer.save()
        id=savecustomer.id
        get_id=Booking.objects.get(id=id)

        savetransaction=Transaction(payment_id=response['razorpay_payment_id'],signature=response['razorpay_signature'],bid=get_id,user=usr)
...
def ticket(request,pk):
    book1=Booking.objects.get(id=pk)
    tran=Transaction.objects.get(bid=pk)
    
    data={}
    data["html_form"]=render_to_string('include/ticket.html', {
        'details':book1,'tran':tran
            },request=request)
